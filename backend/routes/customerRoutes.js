...
const ManageCustomer = () => {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      setCustomers(response.data.reverse()); // Reverse order to show latest first
    } catch (error) {
    ...
    <div className="container mt-4">
      ...
      {editingCustomer && (
        <form onSubmit={handleUpdate} className="mt-4">
          ...
          <button type="button" className="btn btn-secondary ms-2" onClick={() => { setEditingCustomer(null); setFormData({ name: '', address: '', contact: '', email: '' }); }}>
            Cancel
          </button>
        </form>
      )}
